...
class EnhancedModelTrainer:
    ...
    def prepare_training_data(self, vehicle_id: str = None, days_back: int = 30,
                            parameter_names: List[str] = None) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """Prepare comprehensive training data with feature engineering"""
        
        # Define key parameters if not specified
        if parameter_names is None:
            parameter_names = [
                'ENGINE_RPM', 'COOLANT_TEMP', 'ENGINE_LOAD', 'SPEED',
                'FUEL_LEVEL', 'OIL_PRESSURE', 'TRANS_TEMP', 
                'AIR_PRESSURE_FL', 'AIR_PRESSURE_FR'
            ]
        ...
        data_info = {
            ...
            'feature_count': len(engineered_features.columns) - 2,  # Exclude timestamp and session_id
            'date_range': {
                'start': feature_data['timestamp'].min(),
                'end': feature_data['timestamp'].max()
            },
            'missing_data_percentage': engineered_features.isnull().sum().sum() / 
                                     (engineered_features.shape[0] * engineered_features.shape[1]) * 100,
            'parameter_coverage': {param: (feature_data[param].notna().sum() / len(feature_data)) * 100 
                                 for param in parameter_names if param in feature_data.columns}
        }
        
        self.logger.info(f"Engineered {data_info['feature_count']} features from {data_info['total_samples']} samples")
    ...
    def _engineer_features(self, data: pd.DataFrame, parameter_names: List[str]) -> pd.DataFrame:
        ...
        for param in parameter_names:
            if param in data.columns:
                for window in self.feature_windows:
                    ...
                    engineered_data[f'{param}_rolling_mean_{window}'] = data[param].rolling(window=window).mean()
                    engineered_data[f'{param}_rolling_std_{window}'] = data[param].rolling(window=window).std()
                    engineered_data[f'{param}_rolling_min_{window}'] = data[param].rolling(window=window).min()
                    engineered_data[f'{param}_rolling_max_{window}'] = data[param].rolling(window=window).max()
                    
                    # Rolling percentiles
                    engineered_data[f'{param}_rolling_q25_{window}'] = data[param].rolling(window=window).quantile(0.25)
                    engineered_data[f'{param}_rolling_q75_{window}'] = data[param].rolling(window=window).quantile(0.75)
        ...
        if 'AIR_PRESSURE_FL' in data.columns and 'AIR_PRESSURE_FR' in data.columns:
            # Suspension balance
            engineered_data['air_pressure_diff'] = abs(data['AIR_PRESSURE_FL'] - data['AIR_PRESSURE_FR'])
            engineered_data['air_pressure_avg'] = (data['AIR_PRESSURE_FL'] + data['AIR_PRESSURE_FR']) / 2
    ...
    def train_anomaly_detection_model(self, training_data: pd.DataFrame, 
        ...
        # Save model
        model_path = self.models_dir / f"anomaly_{model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
        ...
        self.logger.info(f"Anomaly detection model trained successfully. Anomaly rate: {anomaly_rate:.3f}")
    ...
    def train_predictive_maintenance_model(self, training_data: pd.DataFrame,
        ...
        # Save model
        model_path = self.models_dir / f"predictive_{target_parameter}_{model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
        joblib.dump(pipeline, model_path)
        ...
        self.logger.info(f"Predictive model trained. Test R²: {test_r2:.3f}, Test MSE: {test_mse:.3f}")
    ...
    def train_all_models(self, vehicle_id: str = None, days_back: int = 30) -> Dict[str, Any]:
        ...
        self.logger.info(f"Training completed. {len(results['models_trained'])} models trained, {len(results['errors'])} errors")
        
        return results
    
    def validate_model_performance(self, model_path: str, validation_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    def get_model_recommendations(self, performance_results: Dict[str, Any]) -> List[str]:
        ...
        if 'data_info' in performance_results:
            ...
            if data_info['missing_data_percentage'] > 20:
                recommendations.append("High missing data percentage detected. Consider improving data collection.")
            
            if data_info['total_samples'] < 1000:
                recommendations.append("Limited training data. Collect more data for better model performance.")
        ...
        for model_info in performance_results.get('models_trained', []):
            if model_info['type'] == 'anomaly_detection':
                ...
                if anomaly_rate > 0.3:
                    recommendations.append(f"High anomaly rate ({anomaly_rate:.1%}) detected. Review model parameters.")
                elif anomaly_rate < 0.01:
                    recommendations.append(f"Very low anomaly rate ({anomaly_rate:.1%}). Model may be too conservative.")
                ...
                if test_r2 < 0.5:
                    ...
                    recommendations.append(f"Low predictive accuracy for {param} (R²={test_r2:.2f}). Consider feature engineering.")
        
        # Check for errors
        if performance_results.get('errors'):
            recommendations.append("Some models failed to train. Check data quality and model configurations.")
