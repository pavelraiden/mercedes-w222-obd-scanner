...
class StatusBar(ThemedFrame):
    ...
    def _create_widgets(self):
        ...
        self.separator1 = ctk.CTkLabel(
            self, text="|", text_color=theme_manager.get_color("border"), font=ctk.CTkFont(size=11)
        )
    ...
    def _on_error(self, error_message: str):
        ...
        self.time_label.configure(
            text=f"Ошибка: {error_message[:50]}...", text_color=theme_manager.get_color("error")
        )

        # Сброс через 5 секунд
        self.after(
            5000,
            lambda: self.time_label.configure(
                text="Готов", text_color=theme_manager.get_color("text_secondary")
            ),
        )
...
class MenuBar(ThemedFrame):
    ...
    def _create_widgets(self):
        ...
        self.logo_label = ctk.CTkLabel(
            self, text="", image=icon_manager.create_mercedes_logo((32, 32))
        )
        ...
        # Переключатель темы
        self.theme_button = ThemedButton(self, text="Тема", width=60, command=self._toggle_theme)
...
class MercedesOBDScannerV2:
    ...
    def _create_settings_tab(self, parent):
        ...
        self.theme_combo = ctk.CTkComboBox(
            theme_frame, values=theme_manager.get_theme_names(), command=self._on_theme_selected
        )
