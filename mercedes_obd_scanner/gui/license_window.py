...
class LicenseWindow(ctk.CTkToplevel):
    ...
    def create_widgets(self):
        ...
        self.activate_button = ctk.CTkButton(
            self.main_frame, text="Активировать", command=self.activate
        )
        self.activate_button.pack(pady=10)

        # Активация trial
        self.trial_button = ctk.CTkButton(
            self.main_frame, text="Активировать Trial", command=self.activate_trial
        )
        self.trial_button.pack(pady=5)

        # Информация об оборудовании
        self.hwid_label = ctk.CTkLabel(
            self.main_frame, text=f"Hardware ID: {license_manager.hardware_id}"
        )
