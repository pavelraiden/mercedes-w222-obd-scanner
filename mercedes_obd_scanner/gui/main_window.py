...
class MercedesOBDScanner:
    ...
    def create_menu_bar(self):
        ...
        self.file_menu_btn = ctk.CTkButton(
            self.menu_frame, text="Файл", width=80, command=self.show_file_menu
        )
        self.file_menu_btn.grid(row=0, column=0, padx=5, pady=5)

        self.tools_menu_btn = ctk.CTkButton(
            self.menu_frame, text="Инструменты", width=100, command=self.show_tools_menu
        )
        self.tools_menu_btn.grid(row=0, column=1, padx=5, pady=5)

        self.help_menu_btn = ctk.CTkButton(
            self.menu_frame, text="Справка", width=80, command=self.show_help_menu
        )
        self.help_menu_btn.grid(row=0, column=2, padx=5, pady=5)

        # Индикатор подключения
        self.connection_indicator = ctk.CTkLabel(
            self.menu_frame, text="● Не подключен", text_color="red"
        )
    ...
    def create_status_bar(self):
        ...
        # Статус подключения
        self.status_label = ctk.CTkLabel(self.status_frame, text="Готов к работе", anchor="w")
        self.status_label.grid(row=0, column=0, padx=10, pady=5, sticky="w")

        # Информация о двигателе
        self.engine_info_label = ctk.CTkLabel(
            self.status_frame, text="Двигатель: не определен", anchor="e"
        )
    ...
    def update_connection_status(self, connected: bool):
        ...
        if connected:
            self.connection_indicator.configure(text="● Подключен", text_color="green")
            self.update_status("Подключен к OBD-сканеру")
        else:
            self.connection_indicator.configure(text="● Не подключен", text_color="red")
